...
public class FuncionarioServiceImplTest {
	...
	public void findAll_EncontraUmRegistro_RetornoOk() {
		when(funcionarioRepository.findAll()).thenReturn(retornaLista(cpfTesteOk));		
	...
	public void findById_EncontraRegistro_RetornoOk() {
		when(funcionarioRepository.findOne(cpfTesteOk)).thenReturn(retornaFuncionario(cpfTesteOk));
	...
	private List<Funcionario> retornaLista(Long cpf) {
		return Arrays.asList(retornaFuncionario(cpf));
	}
	
	private Funcionario retornaFuncionario(Long cpf) {
